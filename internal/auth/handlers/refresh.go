...
func (s *AuthServer) RefreshToken(ctx context.Context, refresh *auth.RefreshRequest) (*auth.SignInUserResponse, error) {
	...
	id, err := s.Db.RefershTokenValid(refresh.RefreshToken, refresh.UserAgent, refresh.Ip, userId)
	if err != nil {
		if errors.Is(err, postgresql.ErrUserAgentMismatch) {
			...
			s.logger.Info("refresh token was delete", "userID", id)
	...
	err = s.Db.EditRefreshToken(id, tokens.refresh)
	if err != nil {
		s.logger.Error("failed update refresh token", slog.String("Error", err.Error()))
		return nil, status.Error(codes.Internal, "failed update refresh token")
	}
